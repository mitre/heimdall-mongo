...
function draw_data_table()
{
  ...
  "rowCallback": function( row, data, index ) {
        if ( data.severity == "high" ) {
          $('td:eq(0)', row).html( '<button class="btn btn-danger" style="width:120px" >View Details</button> ' );
        }
        if ( data.severity == "medium") {
          $('td:eq(0)', row).html( '<button class="btn btn-warning" style="width:120px">View Details</button> ' );
        }
        if ( data.severity ==  "low") {
          $('td:eq(0)', row).html( '<button class="btn btn-success" style="width:120px">View Details</button> ' );
        }
        if ( data.severity ==  DATA_NOT_FOUND_MESSAGE) {
          $('td:eq(0)', row).html( '<button class="btn btn-warning" style="width:120px">View Details</button> ' );
        }
      },
        data: dataSet,
...
function update_treemap_colors() {
  ...
  // each block color is updated to its appropirate status color if it does not have status it is replaced to white(FFFFFF)
        ...
        } else if (severity_count['CAT II'] > 0 )
        {
          $('#nist_treemap').children('div').children('div').children('div').children('svg').children('g').each(function() {
        ...
        } else if (severity_count['CAT III'] > 0 )
        {
          $('#nist_treemap').children('div').children('div').children('div').children('svg').children('g').each(function() {
            if ($(this).children('rect').attr('fill') === '#00ff00') {
              $(this).children('rect').css({
