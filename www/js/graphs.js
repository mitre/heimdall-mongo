...
c3.generate({
  ...
  height: 240
    ...
    data: {
        columns: [
            ['CAT I', $(dataSet).filter(function (i,n){return n.severity==='high';}).length],
            ['CAT II', $(dataSet).filter(function (i,n){return n.severity==='medium';}).length],
            ...
            if (!control.nist[j].match(/Rev/))
                {
                    ...
                    record.push(hash);
                    record.push(control.nist[j].match(/[a-zA-Z][a-zA-Z+]-[\d+]/)[0]);
